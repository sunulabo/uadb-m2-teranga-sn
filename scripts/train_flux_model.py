# train_flux_model.py
# Entraînement du modèle de prédiction des flux touristiques — Teranga-SN Eq.12
# Random Forest + MLflow tracking + export du modèle
...
def generer_dataset_historique(n: int = 5000) -> pd.DataFrame:
    """
    Génère un historique simulé de fréquentation touristique.
    Calé sur les réalités saisonnières du Sénégal (haute saison nov-mars).
    """
    ...
    for _ in range(n):
        ...
        # Modèle de flux : haute saison + destinations populaires + bon sentiment
        base_flux  = 200
...
def entrainer_modele():
    """Entraîne le Random Forest et logue les métriques dans MLflow."""
    mlflow.set_tracking_uri(MLFLOW_URI)
    mlflow.set_experiment('teranga_flux_prediction')

    logger.info('Génération du dataset historique...')
    ...
    with mlflow.start_run(run_name=f'rf_flux_{datetime.utcnow().strftime("%Y%m%d_%H%M")}'):
        ...
        # Métriques résumées pour le rapport
        metriques = {
            'rmse': round(rmse, 2),
            'r2':   round(r2, 4),
            'cv_r2': round(cv_r2, 4),
